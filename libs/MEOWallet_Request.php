...
class MEOWallet_Request {
    ...
    public static function verify($url, $apikey, $data) {
        return self::doCallVerify($url, $apikey, $data);
    }
    ...
    public static function doCallVerify($url, $apikey, $data, $post = TRUE) {
        $ch = curl_init();
    ...
    public static function doCallGet($url, $apikey) {
        ...
        if (!in_array($get_result_array->status, array('COMPLETE', 'PENDING', 'FAIL')))
            sleep(5);
